...
\todoc{Maybe steady state is better?} distribution $\vpi =
...
We develop the first \todoc{Maybe leave out efficient? 
What is efficient is a question of interpretation. We 
should maybe rather add a note on the computational complexity.}
...
spectral gap $\gap$ and stationary distribution minorized by $\pimin$. \todoc{Comment on reversibility somewhere?}
...
\begin{equation}
  ...
  ,
\end{equation}
%$(\frac{1}\gap  - 1)\log(2)  \le \tmix \le \frac{1}\gap \log \frac4\pimin$,
and hence it suffices to estimate $\gap$ and $\pimin$. 
\todoc{I would prefer $\ln$ over $\log$ throughout the paper.}
...
\begin{enumerate}
  ...
  \item
    In \cref{sec:rates-lower}, we show in
    \cref{thm:lb-pimin,thm:lb-gap} that $n = \Omega\parens{ (d\log
  ...
  \item
    ...
    \todoc{Is it true from the construction of \cref{thm:combined}
    that if we had a better point estimator
    then we would automatically get an interval estimator
    with the same accuracy?
    This seems to be true.
    This would mean that to close the gap
    one should really work on coming up with better point estimators.
    }
...
\todoc{Explain the organization of the paper?}
%%Performance guarantees play a key role in the analysis and design of machine learning algorithms.
%
