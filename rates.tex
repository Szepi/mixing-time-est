...
%      + \frac{\log\frac{d}{\delta}\cdot\log\frac{n}{\pimin\delta}}{\pimin\gap n}  
  ...
  \end{equation}
\end{theorem}
\todoc{In \eqref{eq:gapbound}, we should be able to merge the second term into the first, right?}
\todod{Oh, I see what you mean---yes, that's right.}
...
This hold \emph{even} if $\pimin$ is with a factor of $1/2$ of the \emph{largest} possible value of $1/d$ that it can take,
i.e., when $\vpi$ is nearly uniform.
\begin{theorem}
  \label{thm:lb-gap}
  There is an absolute constant $c>0$ such that the following holds.
