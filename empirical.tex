...
$\vP$ (Step~\ref{step:P}), then 
compute the so-called group inverse $\giAh$ of $\wh\vA = I - \wh\vP$,
followed by finding the unique stationary
distribution $\hat\vpi$ of $\wh\vP$ (Step~\ref{step:pi}).
The group inverse $\giAh$ of $\wh\vA$ is uniquely defined and if $\wh\vP$ defines an ergodic chain
(which is the case here due to the use of the smoothed estimates),
$\giAh$ can be computed at the cost of inverting an $(d-1)\times (d-1)$ matrix
(cf. Theorem 5.2 in \cite{meyer1975role}).
Further, once given $\giAh$, the unique stationary distribution of $\wh\vP$ can be 
read out from the last row of $\giAh$ (Theorem 5.3 of  \cite{meyer1975role}).
The group inverse will also be used later to compute the sensitivity of $\vpi$
based on a result from \cite{cho2001comparison}.
...
%$\qquad$ $\wh\Delta_{i,j}(\delta) \gets \sqrt{\frac{9\wh{P}_{i,j}(1-\wh{P}_{i,j})\ln(2d/\delta)}{2N_i}}
%  + \frac{4\ln(2d/\delta)+1.5\abs{\alpha-d\alpha\wh{P}_{i,j}}}{N_i}$%,  $(i,j) \in [d]^2$.
%\STATE   $\qquad$ $\wh\Delta_{i,j}(\delta) \gets \min( \wh\Delta_{i,j}(\delta) , \max(\wh{P}_{i,j}, 1-\wh{P}_{i,j}))$,   $(i,j) \in [d]^2$.

%  \STATE Empirical bound for $\max_{i,j}|\wh{P}_{i,j}-P_{i,j}|$:
%  \quad $B(\delta) := \max_{i,j} B_{i,j}(\delta)$.
  %$\wh\Delta_{\vP}(\delta) \gets \sqrt{\sum_{i,j} \wh\Delta_{i,j}^2(\delta)}$,
%  $\wh\Delta^*_{\vP}(\delta) \gets \max_{i,j} \wh\Delta_{i,j}(\delta)$.
...
%    \label{eq:pi-obs-bound}
  ...
  \begin{equation}
    ...
    \Biggr)^{1/2} .
%    \label{eq:gap-obs-bound}
    \notag
  \end{equation}
  \label{step:gap-bound}

%  \STATE Empirical bounds for $\hat{\pi}_i$:
%  \begin{align*}
%    \rho_i(\delta)
%    & := \frac{\wh\kappa B(\delta)}{\hat\pi_i}
%    , \quad i \in [d] ;
%    &
%    \rho_i'(\delta)
%    & := \frac{\wh\kappa B(\delta)}{[\hat{\pi}_i-\wh\kappa B(\delta)]_+}
%    , \quad i \in [d] .
%  \end{align*}

%\STATE $\rho(\delta) \gets \frac12 \max_{i\in \iset{d}} \rho_i(\delta)$; $\rho'(\delta) \gets \frac12 \max_{i\in \iset{d}} \rho_i'(\delta)$.
%\STATE $\hat{\rho}(\delta) \gets 2\max(\rho(\delta),\rho'(\delta)) + \max(\rho(\delta),\rho'(\delta))^2$.
%%$\hat{\rho}(\delta) \gets \rho(\delta) + \rho'(\delta) + \rho'(\delta)\rho(\delta)$. % THIS WAS BUGGY
%\STATE Half-width of interval: $w \gets 
%\hat{\rho}(\delta)+ \parens{ 1+ \hat{\rho}(\delta) } \, \sqrt{ \sum_{i,j} \frac{\hat{\pi}_i}{\hat{\pi}_j} \wh{\Delta}^2_{i,j}(\delta)}
%$.
%\STATE Confidence interval for $\gap$: $U_n(\delta) \gets [\hatgap-w,\hatgap+w]\cap [0,1]$.
\end{algorithmic}
\end{algorithm}

The main result of this section is the following result:
\todoc{Conditions?}
\todod{I think they are fine now. Ergodicity is defined in section 3
now, and we discuss why it already implies the $\pimin>0$ and
$\gap>0$.}
\begin{theorem}
  \label{thm:empirical}
...
\todoc{This needs to be updated once we finalize the results. By how much are these larger?}
...
\begin{theorem}
  ...
  (as $n \to \infty$) \todoc{Do we also care about $\pimin\to 0$, $\gap\to 0$? Do we need then other terms for both? Replicating the form of bounds from \cref{thm:err}?}
  \todod{I don't understand the first question. For the second, we
  could do this, but it would take some non-trivial work\ldots}
  \[
    |\wh{U}|
    =
    O\Parens{
...
%The rest of the section is devoted to the proof of \cref{thm:empirical}.

%In what follows, we will (re-)define  $\wh{\Delta}_{i,j}(\delta)$ to be the RHS of \eqref{eq:P-obs-bound} when $N_i>0$,
% \todoc{Abbrev RHS should be introduced.}
%while we let $\wh{\Delta}_{i,j}(\delta)=\max(\wh{P}_{i,j},1-\wh{P}_{i,j})$ otherwise.
%%Clearly, $\wt{\Delta}_{i,j}(\delta) \ge \wh{\Delta}_{i,j}(\delta)$.
%%In what follows, we calculate with the tighter $\wh{\Delta}_{i,j}(\delta)$. 
%%In fact, the algorithm was presented for $\wt{\Delta}_{i,j}(\delta)$ for easier comprehension only.
%
