...
%and for $\vA=\vI-\vP$, where $\vP$ is ergodic with stationary distribution $\vpi$,
%has the
%following simple
%form: $\vA^\#=(\vI-\vP+\v1\vpi^\t)^{-1}-\v1\vpi^\t$
%\citep{seneta1993sensitivity,meyer1975role}.
}
...
%$\qquad$ $\wh\Delta_{i,j}(\delta) \gets \sqrt{\frac{9\wh{P}_{i,j}(1-\wh{P}_{i,j})\log(2d/\delta)}{2N_i}}
%  + \frac{4\log(2d/\delta)+1.5\abs{\alpha-d\alpha\wh{P}_{i,j}}}{N_i}$%,  $(i,j) \in [d]^2$.
...
%  \todoc{
%    Precisely,
%    what we mean by the $O(\cdot)$ notation is as follows:
%  for a random sequence of nonnegative numbers $(X_n)_n$
%  and another sequence of nonrandom positive numbers $(a_n^\eps)_n$ 
%  indexed by $\eps$ we
%  say that $X_n = O(a_n^\eps)$ holds almost surely as $n\to \infty$
%  if for some universal constant $C>0$, 
%  $\limsup_{n\to \infty} X_n/a_n^\eps \le C$ holds almost surely, regardless of
%  the choice of $\eps$. I think this should be made clear.
%  }
\end{theorem}
...
whose widths shrink at at least as fast as the non-empirical intervals
from \cref{thm:err} and the empirical intervals from
\cref{thm:empirical}.
