...
respectively.
%Assume the Markov chain associated with $\wt{T}$ has a stationary
%distribution denoted by $\tilde\pi$.
%Since $I - T + 1\pi^\t$ is invertible and
%\[
%  (\tilde\pi - \pi)^\t (I - T + 1\pi^\t)
%  = \tilde\pi^\t (\wt{T} - T)
%  ,
%\]
%we can bound $\|\tilde\pi - \pi\|_\infty$ by
%\[
%  \|\tilde\pi - \pi\|_\infty
%  \leq \|(\wt{T} - T)(I - T + 1\pi^\t)^{-1}\|_{\infty\to1}
%  .
%\]
Define
\begin{align*}
